...
const EdspPacketLoss = () => {
  ...
  const fetchData = async () => {
    ...
    const formattedMonthYear = `${date.getFullYear()}-${(date.getMonth() + 1)
      .toString()
      .padStart(2, "0")}`;
    // const response = await fetch(`${baseURL}/api/fetch_edsp_packet_loss`, {
    const response = await fetch(`${phpBaseURL}/fetch_edsp_packet_loss.php`, {
      method: "POST",
  ...
  const handleSave = async () => {
    ...
    if (!recoveryStatus || recoveryStatus.trim() === "") {
      toast.error("Recovery Status is required.");
      return;
    }
    ...
    const requestBody = {
      ...
      trackingId: rowData.Tid,
      recoveryStatus: recoveryStatus,
      remarks: remarks,
      recoveryMonth: monthYear.format("YYYY-MM"),
      packetValue: rowData.value,
    };

    try {
      // const response = await fetch(`${baseURL}/submit_packet_loss`, {
      const response = await fetch(`${phpBaseURL}/submit_packet_loss.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const columns = [
    ...
    { field: "Ship type", headerName: "Ship Type", flex: 1 },
    { field: "Node", headerName: "Node", flex: 1 },
    { field: "value", headerName: "Value", flex: 1 },
    { field: "bucket_1", headerName: "Bucket 1", flex: 1 },
    { field: "final_status", headerName: "Final Status", flex: 1 },
    { field: "final_reason", headerName: "Final Reason", flex: 1 },
    { field: "Recovery status", 
      headerName: "Recovery Status", 
      // flex: 1,
      width: 120
    },
    { field: "Responsible person", headerName: "Responsible Person", flex: 1 },
    { field: "Employee code", headerName: "Employee Code", flex: 1 },
    {
    ...
    { field: "Remarks", headerName: "Remarks", flex: 1 },
    {
      ...
      flex: 1,
      renderCell: (params) => {
        if (params.row["Employee code"] !== null) {
          return (
            <Typography
              component="span"
              variant="body2"
